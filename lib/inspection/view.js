...
export const STATUS_PRESENTATION = {
  ...
  FAIL: { label: "Do not use pending assessment", short: "Do not use", tone: "amber" },
...
export const STATUS_MESSAGE = {
  ...
  FAIL:
    "A serious visible defect was identified. Withdraw the equipment from use until a competent person has assessed the defect and recorded the outcome. This is not a condemnation: the assessment decides whether the equipment returns to service.",
  CRITICAL_FAIL:
    "Tag out, isolate, and remove from operation before any further use. Report to the responsible person.",
};

/**
 * Wording for a status where the operation is already under way.
 *
 * "Do not commence" is an instruction to someone who has not started. Told to a
 * crew with a load in the air it reads as permission to finish the lift, which is
 * the opposite of a hold: the load has to be made safe, not completed.
 */
export const ACTIVE_OPERATION_STATUS_MESSAGE = {
  HOLD_FOR_VERIFICATION:
    "Essential safety information for this operation cannot be verified from the available evidence. STOP / HOLD THE OPERATION — do not continue until the blocking checks below have been completed by a competent person.",
};

/** The short instruction carried on the printed record's verdict line. */
export const HOLD_INSTRUCTION = {
  PRE_START: "DO NOT COMMENCE THE OPERATION",
  ACTIVE: "STOP / HOLD THE OPERATION — DO NOT CONTINUE",
};
...
/**
 * Accepts an operation context, a whole result, or a bare state string, so every
 * caller can pass whatever it already holds.
 *
 * @param {any} context
 * @returns {string} the operation state, uppercased, or ""
 */
function operationState(context) {
  if (typeof context === "string") return context.trim().toUpperCase();
  const state = text(context?.state) || text(context?.operation_context?.state);
  return state.toUpperCase();
}

/**
 * @param {any} context an operation context, a result, or a state string
 * @returns {boolean} true where the photographed operation is already under way
 */
export function isActiveOperation(context) {
  return operationState(context) === "OPERATION_ACTIVE";
}

/**
 * @param {any} context an operation context, a result, or a state string
 * @returns {string} the hold instruction matching how far the work has progressed
 */
export function getHoldInstruction(context) {
  return isActiveOperation(context) ? HOLD_INSTRUCTION.ACTIVE : HOLD_INSTRUCTION.PRE_START;
}

/**
 * @param {string | undefined | null} status
 * @param {any} [operationContext] an operation context, a result, or a state string
 */
export function getStatusMessage(status, operationContext) {
  const key = /** @type {keyof typeof STATUS_MESSAGE} */ (status);
  if (isActiveOperation(operationContext) && ACTIVE_OPERATION_STATUS_MESSAGE[key]) {
    return ACTIVE_OPERATION_STATUS_MESSAGE[key];
  }
  return STATUS_MESSAGE[key] || STATUS_MESSAGE.CONDITIONAL_PASS;
}
...
/**
 * The one narrative sentence, built from the validated result.
 *
 * Every surface that states counts in prose calls this, so the screen, the printed
 * report and anything generated from them cannot disagree with each other or with
 * the header tiles — they are all reading the same validated arrays.
 *
 * @param {InspectionResult | LegacyInspectionResult | null | undefined} result
 * @returns {string}
 */
export function getResultNarrative(result) {
  const counts = getCounts(result);
  const risk = getRiskDisplay(result);

  const parts = [plural(counts.hazards, "visible hazard")];
  if (counts.verifications > 0) {
    const blocking = counts.blocking > 0 ? ` (${counts.blocking} blocking)` : "";
    parts.push(`${plural(counts.verifications, "verification point")}${blocking}`);
  }
  if (counts.controls > 0) parts.push(plural(counts.controls, "visible compliant control"));

  const riskText = risk.pending ? `risk index ${risk.display} — ${risk.caption?.toLowerCase()}` : `risk index ${risk.display}/100`;
