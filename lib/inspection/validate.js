...
function classifyHazard(raw, log) {
  ...
  // a frayed wire rope lying on the deck is still a frayed wire rope.
  if (!MECHANICAL_DAMAGE_RE.test(visibleEvidence) && !LOAD_PATH_RE.test(evidenceProse)) {
    if (HOUSEKEEPING_RE.test(evidenceProse) || (SCENE_OBJECT_RE.test(evidenceProse) && SCENE_PLACEMENT_RE.test(evidenceProse))) {
      return {
        demoted:
          "Housekeeping or scene condition rather than the declared equipment, its load path or its own condition.",
      };
    }
  }
